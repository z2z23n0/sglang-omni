...
This page is the central user-facing view of model support. A status applies to
the documented configuration, not every theoretical checkpoint, precision, or
hardware combination. "Not yet recorded" means the project has not documented
a validated hardware configuration; it is not a claim that the model is
unsupported.
...
| Model | Task | Endpoint | Pipeline | Streaming | Validated hardware | Status | Cookbook |
|---|---|---|---|---|---|---|---|
| Higgs Audio v3 | TTS | `/v1/audio/speech` | preprocessing → TTS engine → vocoder | Yes | 1× H100 | CI tested | [Higgs TTS](./cookbook/higgs_tts.md) |
...
| dots.tts | TTS | `/v1/audio/speech` | preprocessing → backbone/acoustic tail → AudioVAE | Yes | 1× H100 | Performance qualified | [dots.tts](./cookbook/dots_tts.md) |
| ZONOS2 | TTS | `/v1/audio/speech` | preprocessing → speaker encode → TTS engine → vocoder | Yes | 1× H100 | Performance qualified | [ZONOS2](./cookbook/zonos2.md) |
| MiniMax Music 3 | Music | `/v1/audio/speech` | autoregressive music engine → DIT/DAC decode | No | 1× H200 | Performance qualified | [MiniMax Music 3](./cookbook/minimax_music3.md) |
| Qwen3-ASR | ASR | `/v1/audio/transcriptions` | audio preprocessing → ASR engine → response formatting | Yes | RTX 4090 24 GB; 1× H100 CI | CI tested | [Qwen3-ASR](./cookbook/qwen3_asr.md) |
